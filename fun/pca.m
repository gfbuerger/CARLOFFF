function [E PC ev lmb D] = pca(x, trunc="nEOF", lNaN=true)

   ## usage:  [E PC ev lmb D] = pca(x [,trunc [,lNaN]])
   ## 
   ...
   if lNaN
	 ...
	 wPC = wPC(:,is) ; D = D(is,is) ; E = E(:,is) ;
      ...
      J = all(isfinite(nan2lim(x, 0))) ;
      if 1 | np > 20 # octave bug
	 if exist(sfile = "data/svd.mat", "file") == 2
	    clear x ;
	    load(sfile) ;
	 else
	    [PC, D, E] = svd(nan2lim(x, 0), np) ;
	    save(sfile, "PC", "D", "E") ;
	 endif
      else
	 [PC, D, E] = svds(nan2lim(x, 0), np) ;
	 D = real(D) ;
	 [~, is] = sort(diag(D)) ;
	 wPC = wPC(:,is) ; D = D(is,is) ; E = E(:,is) ;
      endif
...
## usage: y = nan2lim (x, lim)
##
## replace NaNs with lim
function y = nan2lim (x, lim)
   y = x ;
   y(isnan(y)) = lim ;
endfunction
